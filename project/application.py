...
def index():
    ...
    if num == 0:
        ...
        books = db.execute("SELECT * FROM books WHERE id = :f1",f1=int(f1))
        for book in books:
            title.append(book["title"])
            author.append(book["author"])
            price.append(book["price"])
            DoP.append(book["DoP"])
        books = db.execute("SELECT * FROM books WHERE id = :f2",f2=int(f2))
        for book in books:
            title.append(book["title"])
            author.append(book["author"])
            price.append(book["price"])
            DoP.append(book["DoP"])
        books = db.execute("SELECT * FROM books WHERE id = :f3",f3=int(f3))
        for book in books:
            title.append(book["title"])
            author.append(book["author"])
            price.append(book["price"])
...
def history():
    if session["user_state"] == 2:
        session.clear()
        return apology("Your account is suspended")
    else:
        """Show history"""
...
def register():
    ...
    if request.method == "POST":
        ...
        # Redirect user to home page
        return apology("Your serial number is "+str(serial),200)
    # User reached route via GET (as by clicking a link or via redirect)
    else:
...
def ban():
    ...
    if request.method == "GET":
        ...
        if len(rows) != 1 :
            ...
            return apology("User has been banned successfully",200)
...
def forget_password():
    ...
    if request.method == "GET":
        ...
        if not request.form.get("username"):
            return apology("must provide username", 403)
        elif not request.form.get("serial"):
            return apology("must provide serial number", 403)

        rows = db.execute("SELECT * FROM users WHERE username = :name",name=request.form.get("username"))
        if int(request.form.get("serial"))==rows[0]["serial"]:
            db.execute("UPDATE users SET hash = :password WHERE username = :username",username = request.form.get("username"), password = generate_password_hash(request.form.get("serial")))
            return apology("Your password has been updated to your serial number",200)
        else:
            return apology("Wrong serial number",200)
...
def balance():
    if session["user_state"] == 2:
        ...
        if request.method == "POST":
            ...
            update_session()
            return apology("balance has been added successfully",200)
    ...
    if session["user_state"] == 2:
        ...
        if request.method == "GET":
            ...
            if  not check_password_hash(rows[0]["hash"], request.form.get("password")):
                ...
                update_session()
                #alert
                return apology("Name changed successfully",200)
    ...
    if session["user_state"] == 2:
        ...
        if request.method == "GET":
            ...
            if not request.form.get("title"):
                ...
                if searchby == "Title":
                    rows = db.execute("SELECT * FROM books WHERE title LIKE :title ", title='%'+request.form.get("title")+'%')
                elif searchby == "Author":
                    rows = db.execute("SELECT * FROM books WHERE author LIKE :author", author='%'+request.form.get("title")+'%')
                elif searchby == "Date of publish":
                    rows = db.execute("SELECT * FROM books WHERE DoP LIKE :dop", dop='%'+request.form.get("title")+'%')
                title=[]
                author=[]
                DoP=[]
                genre=[]
